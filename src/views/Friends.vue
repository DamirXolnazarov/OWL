    ...
    <div class="people flex flex-col items-center gap-[15px]">
      <div v-for="user in filteredUsers" :key="user.id" class="person-card flex flex-row justify-between">
        <div class="left flex flex-col items-start justify-center">
          ...
          <span class="cursor-pointer"><a :href="`mailto:${user.email}`">{{ user.email }}</a></span>
        </div>
        <div class="right gap-[25px] flex flex-row items-center justify-between">
          <span>Reading: {{ user.stats.booksInProgress.name }}</span>
          <span
            >Total XP:
            {{
              user.stats.quizzesCompleted * 10 +
              user.stats.pagesRead * 10 +
              user.stats.booksCompleted * 100
            }}</span
          >

          <button class="btn follow" @click="toggleFollow(user.email)" :disabled="!userData">
            {{ userData?.friends?.includes(user.email) ? 'Following' : 'Follow' }}
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async toggleFollow(emailToFollow) {
      ...
      if (this.userData.friends.includes(emailToFollow)) {
        await updateDoc(userRef, { friends: arrayRemove(emailToFollow) })
      } else {
        await updateDoc(userRef, { friends: arrayUnion(emailToFollow) })
      }
